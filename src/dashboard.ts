...
function renderDatasetChart(rows: any[], profile: any): string {
  ...
  if (profile.shape === "multi_metric" && profile.metrics.length >= 2) {
    ...
    if (comparisonDimension && uniqueNonEmptyValues(rows, comparisonDimension).length <= 12) {
      return renderComboChart(rows, comparisonDimension, metric, profile.metrics.find((candidate: string) => candidate !== metric) ?? metric);
    }
  }
  if (profile.shape === "multi_metric" && profile.metrics.length >= 2) return renderMetricScorecard(rows, profile.metrics);
...
function uniqueNonEmptyValues(rows: any[], column: string): string[] {
  return unique(rows.map(row => String(row[column] ?? "").trim()).filter(Boolean));
}

function chooseSecondaryCategoricalDimension(rows: any[], columns: string[], primary?: string, excluded: string[] = []): string | undefined {
  const blocked = new Set([primary, ...excluded].filter(Boolean));
  return columns.find(column => {
    if (blocked.has(column)) return false;
...
function chooseHeatmapDimensions(rows: any[], columns: string[]): [string, string] | undefined {
...
function hasSufficientTimeSeries(rows: any[], dimension: string, metric: string): boolean {
  const grouped = aggregateRowsByDimension(rows, dimension, metric);
  return grouped.length >= 3;
}

function shouldUseCircularChart(rows: any[], dimension: string, metric: string): boolean {
  ...
  if (categoryCount < 2 || categoryCount > 6) return false;
  const total = grouped.reduce((sum, row) => sum + numericValue(row[metric]), 0);
  const largestShare = total ? Math.max(...grouped.map(row => numericValue(row[metric]) / total)) : 0;
  return largestShare < 0.85;
}

function shouldUseScatterChart(rows: any[], xMetric: string, yMetric: string): boolean {
...
function shouldUseComboChart(rows: any[], dimension: string, barMetric: string, lineMetric: string): boolean {
  const grouped = aggregateRowsByDimension(rows, dimension, barMetric, [lineMetric]);
  return grouped.length >= 3 && grouped.length <= 18;
}

function shouldUseStackedComposition(rows: any[], timeDimension: string, categoryDimension: string, metric: string): boolean {
...
function shouldUseMapChart(rows: any[], columns: string[], geoColumn?: string): boolean {
  if (findLatitudeColumn(columns) && findLongitudeColumn(columns)) return true;
  if (!geoColumn) return false;
  const cardinality = uniqueNonEmptyValues(rows, geoColumn).length;
  return cardinality >= 2 && cardinality <= 30;
}

function renderGovernedCategoryChart(rows: any[], dimension: string, metric: string, allowCircular: boolean): string {
  if (!dimension || !metric || !rows.length) return "<p>No category chart data.</p>";
...
function aggregateRowsByTwoDimensions(rows: any[], firstDimension: string, secondDimension: string, metric: string): any[] {
  const groups = new Map();
  for (const row of rows) {
...
function renderHeatmapChart(rows: any[], firstDimension: string, secondDimension: string, metric: string): string {
  const aggregated = aggregateRowsByTwoDimensions(rows, firstDimension, secondDimension, metric);
  if (!aggregated.length) return "<p>No heatmap data.</p>";
...
function renderStackedCompositionChart(rows: any[], timeDimension: string, categoryDimension: string, metric: string): string {
  const aggregated = aggregateRowsByTwoDimensions(rows, timeDimension, categoryDimension, metric);
  if (!aggregated.length) return "<p>No stacked composition data.</p>";
