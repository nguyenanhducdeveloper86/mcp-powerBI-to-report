...
function buildDatasetAwareSummary(profiles, joinPlan) {
    ...
    if (!nonEmpty.length)
        return "No rows returned from the selected semantic models.";
...
// Intent detection
// ═══════════════════════════════════════════════════════
function detectReportIntent(question, rows, columns) {
    ...
    const dimCols = columns.filter(c => !["Scope", "Dimension", "Member", "DataSource", "EvidenceRole", "WorkspaceName", "SemanticModelName"].includes(c) &&
...
function isMonthLikeColumn(col, rows) {
    const n = normalizeForMatch(col);
    if (["yearmonth", "month", "period", "thang", "date", "ngay"].some(t => n.includes(t)))
        return true;
    return rows.some(r => {
        const v = r[col];
...
// Summary (mode-aware)
// ═══════════════════════════════════════════════════════
function buildSummary(intent, rows, columns, analysis) {
    if (!rows.length)
...
// Insights (mode-aware)
// ═══════════════════════════════════════════════════════
function buildInsights(intent, analysis, rows, columns, business) {
    const vi = intent.language === "vi";
...
// ═══════════════════════════════════════════════════════
function buildBusinessInsightModel(intent, analysis, rows, columns) {
    const vi = intent.language === "vi";
...
function buildDataProfile(rows, columns, metric) {
    ...
    for (const item of [
        ...
        if (!findColumnByTokens(columns, tokens))
            missing.push(label);
    }
    return {
...
function detectDimensionColumns(rows, columns) {
    if (columns.includes("Dimension") && columns.includes("Member")) {
        ...
        if (scopedDims.length)
            return scopedDims;
    }
    return columns.filter(c => {
        if (["Scope", "Dimension", "Member", "DataSource", "EvidenceRole", "WorkspaceName", "SemanticModelName"].includes(c))
            return false;
        if (rows.some(r => typeof r[c] !== "string"))
            return false;
        const uniqueCount = unique(rows.map(r => String(r[c] ?? "").trim()).filter(Boolean)).length;
        return uniqueCount >= 2;
    });
}
function buildContributionInsights(rows, columns, metric, dataProfile) {
...
function contributionInsightFromGrouped(dimension, grouped, total, metric) {
    if (grouped.length < 2)
        return undefined;
    const top = grouped[0];
...
function buildCrossDimensionPockets(rows, columns, metric, dataProfile) {
    if (columns.includes("Dimension") && columns.includes("Member")) {
        return rows
...
function buildPocketFromRow(dimension, label, row, metric, rows) {
    if (!label)
        return undefined;
    const total = businessMetricTotal(rows, metric);
...
function buildRiskAndOpportunityWatch(rows, columns, metric, dataProfile) {
...
function buildNextBestQuestions(context) {
...
function renderBusinessInsightSection(business) {
    ...
    if (!cards.length && !contributions.length && !pockets.length && !risks.length)
        return "";
...
function renderContributionLayer(contributions) {
...
function renderPocketLayer(pockets) {
    if (!pockets.length)
        return "";
    return `<article class="panel">
...
function renderRiskLayer(risks) {
    if (!risks.length)
        return "";
    return `<article class="panel">
...
function renderDataProfileLayer(profile, nextQuestions) {
    return `<article class="panel">
...
function renderDecisionCards(rows) {
    if (!rows?.length)
        return "<p>No decision rows available.</p>";
...
function renderEvidenceGateSection(input) {
...
function renderEvidenceQueryBlock(result) {
    if (result.error) {
        return `<article class="panel">
...
function prioritizeEvidenceColumns(columns, dimensions) {
    const priority = [
        ...dimensions,
        "FocusRevenue",
        "AvgRevenue",
...
function renderDatasetEvidenceSection(input) {
    const datasets = input.datasets ?? [];
    const profiles = input.datasetProfiles ?? [];
    if (!datasets.length || !profiles.length)
        return "";
    const joinPlan = input.joinPlan ?? {};
...
function renderDatasetBlock(dataset, profile) {
...
function renderDatasetChart(rows, profile) {
    const metric = profile.primaryMetric;
    const dimension = profile.primaryDimension;
    if (!rows.length)
...
function renderMetricScorecard(rows, metrics) {
    const selected = metrics.slice(0, 6);
    if (!selected.length)
        return "<p>No numeric measures available.</p>";
...
function renderCrossDatasetInsightBoard(datasets, profiles) {
...
function datasetReadout(profile) {
    if (profile.shape === "time_series")
        return `Use this block for trend, seasonality, extremes, and run-rate questions at ${profile.grain} grain.`;
    if (profile.shape === "categorical_ranking")
        return `Use this block for top/bottom, concentration, and contribution decisions by ${profile.primaryDimension}.`;
    if (profile.shape === "cross_dimension")
        return `Use this block to find business pockets across dimensions instead of reading dimensions independently.`;
    if (profile.shape === "multi_metric")
        return "Use this block as a driver scorecard; it needs a dimension or time grain for deeper diagnosis.";
    if (profile.shape === "empty")
        return "This evidence source returned no rows and should be treated as a data gap.";
    return "Use this block as supporting evidence; add dimensions or metrics to make it decision-ready.";
}
function topDatasetEvidence(rows, dimension, metric) {
    ...
    const top = grouped[0];
    if (!top)
        return "No comparable members returned";
...
function decisionCueForShape(profile) {
    if (profile.shape === "time_series")
        return "Validate whether peak/trough is repeatable, seasonal, or operational before acting.";
    if (profile.shape === "categorical_ranking")
        return "Prioritize top contributors, then check margin/risk before scaling.";
    if (profile.shape === "cross_dimension")
        return "Drill into the strongest pockets; these are usually where CEO-level actions live.";
    if (profile.shape === "multi_metric")
        return "Use as a driver scan; request segmentation to locate where the driver matters.";
    if (profile.shape === "empty")
        return "Do not use this source for decisions until the query/model returns data.";
    return "Use as audit evidence, not the main decision layer.";
}
function missingEvidenceForProfile(profile) {
    const missing = [];
    if (!profile.timeDimensions?.length)
        missing.push("time grain");
    if (!profile.categoricalDimensions?.length)
        missing.push("business dimension");
    if ((profile.metrics ?? []).length < 2)
        missing.push("secondary driver metric");
    return missing.length ? missing.join(", ") : "root-cause fields such as plan, margin, inventory, discount, campaign";
}
function aggregateRowsByDimension(rows, dimension, metric, extraMetrics = []) {
    const groups = new Map();
    ...
    for (const row of rows) {
        ...
        if (!label)
            continue;
        if (!groups.has(label))
            groups.set(label, Object.fromEntries(metrics.map(column => [column, 0])));
...
function findBestCategoricalColumn(rows, columns, excluded = []) {
    ...
    return columns.find(column => {
        if (excludedSet.has(column) || metadata.has(column) || isMonthLikeColumn(column, rows))
            return false;
...
function findGeoColumn(rows, columns) {
...
function scaleIndex(index, count, left, right) {
    if (count <= 1)
        return (left + right) / 2;
    return left + (index / (count - 1)) * (right - left);
}
function scaleValue(value, min, max, lower, upper) {
...
function renderYAxisLabels(bounds, box, metric) {
...
function collapseSmallSlices(rows, dimension, metric, limit) {
...
function shortNumber(metric, value) {
...
// ═══════════════════════════════════════════════════════
function buildDatasetKpis(profiles) {
...
function buildKpis(rows, columns, intent, analysis) {
    if (intent?.mode === "month-extremes" && analysis) {
    ...
    const dim = intent?.primaryDimension;
    if (!metric || !rows.length)
        return [{ label: "Rows", value: formatNumber(rows.length), tone: "blue" }];
    const total = rows.reduce((s, r) => s + numericValue(r[metric]), 0);
    const avg = total / rows.length;
    const sorted = [...rows].sort((a, b) => numericValue(b[metric]) - numericValue(a[metric]));
    const top = sorted[0];
...
function getChartTitle(intent, analysis) {
    if (!intent)
...
// ═══════════════════════════════════════════════════════
function renderRankedBars(rows, dim, metric, topN) {
...
function renderTrendBars(rows, dim, metric) {
    if (!dim || !metric || !rows.length)
        return "<p>No chart data.</p>";
    const sorted = [...rows].sort((a, b) => compareDimension(a[dim], b[dim]));
    const values = sorted.map(r => numericValue(r[metric]));
...
function renderDistributionBars(rows, dim, metric) {
    return renderGenericBars(rows, dim, metric, 20);
}
function renderGenericBars(rows, dim, metric, limit = 15) {
    if (!dim || !metric || !rows.length)
        return "<p>No chart data.</p>";
...
function renderNativeChartGallery(input) {
...
function renderNativeCard(title, subtitle, chart) {
    return `<article class="native-card"><h3>${escapeHtml(title)}</h3><p>${escapeHtml(subtitle)}</p>${chart}</article>`;
}
function renderLineChart(rows, dimension, metric) {
    if (!dimension || !metric || !rows.length)
        return "<p>No line chart data.</p>";
...
function renderComboChart(rows, dimension, barMetric, lineMetric) {
    if (!dimension || !barMetric || !lineMetric || !rows.length)
        return "<p>No combo chart data.</p>";
...
function renderPieChart(rows, dimension, metric) {
    return renderPieLikeChart(rows, dimension, metric, false);
}
function renderDonutChart(rows, dimension, metric) {
    return renderPieLikeChart(rows, dimension, metric, true);
}
function renderPieLikeChart(rows, dimension, metric, donut) {
    if (!dimension || !metric || !rows.length)
        return `<p>No ${donut ? "donut" : "pie"} chart data.</p>`;
    ...
    if (!total)
        return `<p>No positive values for ${donut ? "donut" : "pie"} chart.</p>`;
    const colors = chartPalette();
...
function renderScatterChart(rows, xMetric, yMetric, labelDimension) {
    if (!xMetric || !yMetric || !rows.length)
        return "<p>No scatter plot data.</p>";
...
function renderMapChart(rows, columns, metric, geoColumn) {
...
function renderGeoBubbleMap(rows, latColumn, lonColumn, metric, labelColumn) {
    ...
    if (!data.length)
        return "<p>No latitude/longitude rows available for map.</p>";
...
function selectMonthlyRows(rows) {
    ...
    if (rows.some(r => r.Scope !== undefined))
        return scoped;
...
function riskRead(row, invCol, discCol, mktCol, msCol) {
    const parts = [];
    if (invCol && numericValue(row[invCol]) > 42)
        parts.push("inventory high");
    if (discCol && numericValue(row[discCol]) > numericValue(row.Revenue_BillionVND) * 0.04)
        parts.push("discount heavy");
    if (mktCol && numericValue(row[mktCol]) > numericValue(row.Revenue_BillionVND) * 0.025)
        parts.push("marketing intensity high");
    if (msCol && numericValue(row[msCol]) < 10)
...
function averageContributionByMonth(rows, monthCol, dimCol, dimVal, metricCol) {
    ...
    for (const r of rows) {
        const m = String(r[monthCol] ?? "").trim();
        if (!m || String(r[dimCol] ?? "").trim() !== dimVal)
            continue;
        totals.set(m, (totals.get(m) ?? 0) + numericValue(r[metricCol]));
    }
    return average([...totals.values()]);
...
function formatMetricValue(metric, value) {
    ...
    if (n.includes("billionvnd"))
        return `${formatNumber(value)} tỷ VND`;
    if (n.includes("millionvnd"))
