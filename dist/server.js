...
import { buildEvidenceGateResult, buildEvidencePlan, schemaColumnNames, schemaScanQuery, shouldRunEvidenceGate } from "./evidenceGate.js";
...
server.registerTool("diagnose_powerbi_mcp", {
...
server.registerTool("list_semantic_models_in_workspace", {
...
server.registerTool("get_known_workspace_catalog", {
...
server.registerTool("plan_multi_semantic_report", {
...
server.registerTool("execute_multi_semantic_report", {
...
server.registerTool("execute_dax_query", {
...
server.registerTool("execute_dax_report_query", {
...
server.registerTool("execute_dax_dashboard_query", {
...
function inferExecutiveIntent(question) {
    const q = normalizeText(question);
    if (/\b(forecast|du bao|du kien|thang toi|quy toi|nam toi)\b/.test(q))
        return "forecast_risk";
    if (/\b(if|neu|scenario|gia su|tang|giam|discount|price)\b/.test(q))
        return "scenario_simulation";
    if (/\b(tinh nao|province|region|dau tu|opportunity|co hoi)\b/.test(q))
        return "opportunity_prioritization";
    if (/\b(model|product|scale|portfolio|san pham)\b/.test(q))
        return "portfolio_decision";
    if (/\b(dealer|daily|showroom|salesperson|performance)\b/.test(q))
        return "performance_management";
    if (/\b(tai sao|why|vi sao|giam|gap|variance|lech)\b/.test(q))
        return "variance_decomposition";
    if (/\b(thang|month|cao nhat|thap nhat|highest|lowest)\b/.test(q))
...
function decisionNeedForIntent(intent) {
    const map = {
...
function blocksForIntent(intent) {
...
function inferJoinKeys(question, evidence, fields) {
...
function inferEvidenceRole(modelName, evidence) {
    const name = normalizeText(modelName);
    const text = normalizeText((evidence ?? []).join(" "));
    if (name.includes("marketing") || text.includes("marketing") || text.includes("campaign"))
        return "marketing";
    if (name.includes("inventory") || text.includes("inventory") || text.includes("stock"))
        return "inventory";
    if (name.includes("finance") || text.includes("margin") || text.includes("profit"))
        return "finance";
    if (name.includes("dealer") || text.includes("dealer"))
        return "dealer";
    if (name.includes("sale") || text.includes("revenue"))
        return "sales";
...
function validateMultiSemanticEvidence(options) {
    ...
    for (const source of emptySources)
        warnings.push(`${source.semanticModelName} returned zero rows.`);
...
function extractRows(result) {
    ...
    if (Array.isArray(result.rows))
        return rowsFromRowsColumns(result.rows, Array.isArray(result.columns) ? result.columns : undefined).filter(isRecord);
    if (isRecord(result.result))
        return extractRows(result.result);
    if (isRecord(result.data))
        return extractRows(result.data);
    return [];
}
function rowsFromRowsColumns(rows, columns) {
    if (!columns)
